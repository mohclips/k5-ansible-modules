...
def k5_update_router_remove_ports(module):
    """Replaces the Routes on a K5 router"""
    ...
    for port in ports:
        ...
        if port_id != "":
            ...
            url = endpoint + '/v2.0/routers/' + router_id + '/remove_router_interface'
            ...
            if k5_debug:
                module.exit_json(changed=True, msg="Router ports remove Successful", k5_router_facts=response.json(), debug=k5_debug_out )


    module.exit_json(changed=True, msg="Router ports remove Successful", k5_router_facts=response.json()['router'])
