...
def k5_update_router_routes(module):
    ...
    k5_debug_add('routes: {0}'.format(routes))

    session = requests.Session()

    headers = {'Content-Type': 'application/json', 'Accept': 'application/json', 'X-Auth-Token': auth_token }

    url = endpoint + '/v2.0/routers/' + router_id
  
    # In order to remove all routes we must pass a null when issuing the command

    query_json = {"router": { "routes": routes_dict }}
    ...
    if k5_debug:
        module.exit_json(changed=True, msg="Routes Update Successful", k5_router_facts=response.json()['router'], debug=k5_debug_out )

    module.exit_json(changed=True, msg="Routes Update Successful", k5_router_facts=response.json()['router'])
